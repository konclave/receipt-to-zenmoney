...
<script lang="ts">
  ...
  let cleanupConfirmPeriod = $state<{
    period: number | "all";
    txCount: number;
    bytes: number;
  } | null>(null);
  ...
  onMount(async () => {
    ...
    accounts = savedAccounts.sort((a, b) => a.title.localeCompare(b.title));
    if (cats.length > 0)
      lastSyncDate = new Date(cats[0].syncedAt).toLocaleDateString();
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (selectedAccountId)
        await saveSettings({ zenmoneyAccountId: selectedAccountId });
  ...
  async function handleReloadCategories() {
    ...
    try {
      ...
      await saveSettings({
        ...
        zenmoneyUserId: userId,
      });
      categoryCount = cats.length;
      lastSyncDate = new Date().toLocaleDateString();
      accounts = response.account.sort((a, b) =>
      ...
      if (response.account.length === 1) {
        ...
        await saveSettings({
          zenmoneyAccountId: response.account[0].id,
        });
      }
    } catch (e) {
      error = String(e);
  ...
  function handleCleanupSelect(period: number | "all") {
    ...
    if (period === "all") {
      const totalTxCount = storageStats.byYear.reduce(
        (s, y) => s + y.txCount,
        0,
      );
      cleanupConfirmPeriod = {
        period: "all",
      ...
      cleanupConfirmPeriod = {
        period,
        txCount: yearStat.txCount,
        bytes: yearStat.bytes,
      };
    }
  }
  ...
  <section>
    ...
    <div class="provider-tabs">
      <button
        ...
        onclick={() => (aiProvider = "anthropic")}>Anthropic</button
      >
      <button
        type="button"
        class="provider-tab"
        class:active={aiProvider === "openrouter"}
        onclick={() => (aiProvider = "openrouter")}>OpenRouter</button
      >
    </div>
    ...
    {#if aiProvider === "anthropic"}
      <label for="claude-key">
        ...
        <span
          class="key-dot"
          class:set={claudeApiKeySaved}
          role="img"
          aria-label={claudeApiKeySaved ? "saved" : "not saved"}>●</span
        >
      </label>
      <input
        id="claude-key"
    ...
    {:else}
      <label for="or-key">
        ...
        <span
          class="key-dot"
          class:set={openrouterApiKeySaved}
          role="img"
          aria-label={openrouterApiKeySaved ? "saved" : "not saved"}>●</span
        >
      </label>
      <input
        id="or-key"
      ...
      {:else}
        ...
        <p class="hint">
          {orModels.length} vision-capable models available
        </p>
      {/if}
    {/if}
  </section>
  ...
  {#if accounts.length > 1}
    <section>
      ...
      <button
        class="btn-primary"
        onclick={handleSave}
        disabled={saving || !accountDirty}>Save Account</button
      >
    </section>
  {/if}
  ...
  <section>
    ...
    <div class="storage-stats">
      ...
      <span class="storage-size"
        >{storageStats ? formatBytes(storageStats.totalBytes) : "—"}</span
      >
      {#if storageStats}
        <span class="hint">
          {storageStats.byYear.length}
          {storageStats.byYear.length === 1 ? "year" : "years"} · {storageStats.byYear.reduce(
            (s, y) => s + y.txCount,
            0,
          )} transactions
        </span>
