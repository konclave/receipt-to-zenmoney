...
export async function getConfiguredZenMoneyToken(forceRefresh = false): Promise<string> {
  const settings = await getSettings();
  if (resolveZenMoneyAuthMode(settings.zenmoneyAuthMode, isOAuthEnabled()) === 'manual') {
...
export async function getZenMoneyAuthMode(): Promise<'manual' | 'oauth'> {
  return resolveZenMoneyAuthMode((await getSettings()).zenmoneyAuthMode, isOAuthEnabled());
}
