<script lang="ts">
  ...
  const totalTxCount = $derived(
    stats.byYear.reduce((s, y) => s + y.txCount, 0),
  );
</script>
    ...
    <div class="modal-body">
      {#each stats.byYear as yearStat}
        <div class="year-row">
          <div class="year-info">
            ...
            <span class="meta"
              >{yearStat.txCount} transactions · {formatBytes(
                yearStat.bytes,
              )}</span
            >
          </div>
          <button class="btn-delete" onclick={() => onselect(yearStat.year)}
            >Delete</button
          >
        </div>
      {/each}
      <div class="year-row all-row">
        <div class="year-info">
          ...
          <span class="meta"
            >{totalTxCount} transactions · {formatBytes(stats.totalBytes)}</span
          >
        </div>
        <button class="btn-delete-all" onclick={() => onselect('all')}
          >Delete all</button
        >
      </div>
    </div>
