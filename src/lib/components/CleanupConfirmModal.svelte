<script lang="ts">
  ...
  const title = $derived(
    period === 'all' ? 'Delete all data?' : `Delete ${period} data?`,
  );
...
>
  ...
  >
    ...
    <div class="modal-body">
      {#if !confirmingDelete}
        <p class="description">
          Download a backup of this period before deleting? The file can be
          reimported later.
        </p>
        <button
          class="btn-primary"
          onclick={() => onconfirm({ withBackup: true })}
        >
          <span>Download backup & delete</span>
          <span class="btn-hint">Saves {backupFilename} first</span>
        </button>
        <button
          class="btn-delete-secondary"
          onclick={() => {
      ...
      {:else}
        ...
        <button
          ...
          onclick={() => onconfirm({ withBackup: false })}
        >
